...
import { LumenComponentPropertyValueParser } from './component-property-value-parser';
import { LumenComponentPropertyReferenceDecoder } from './component-property-reference-decoder';
import type {
...
export class LumenComponentPropertySchema {
    /**
     * @description Prefab 内嵌条目与节点、组件引用解码器。
     */
    private readonly _referenceDecoder = new LumenComponentPropertyReferenceDecoder();

    /**
     * @description 公共属性值校验与序列化形状转换器。
     */
    private readonly _valueParser = new LumenComponentPropertyValueParser();
    ...
    private _applyCurveRange(
        ...
        if (Array.isArray(keysMinRaw) || Array.isArray(keysMaxRaw)) {
            ...
            if (record.multiplier !== undefined) {
                target.multiplier = this._valueParser.asNumber(record.multiplier, `${apiName}.multiplier`);
        ...
        for (const key of ['mode', 'constant', 'constantMin', 'constantMax', 'multiplier'] as const) {
            if (record[key] !== undefined) {
                target[key] = this._valueParser.asNumber(record[key], `${apiName}.${key}`);
            }
        }
    ...
    private _applyGradientRange(
        ...
        if (rawValue != null && typeof rawValue === 'object' && !Array.isArray(rawValue)) {
            ...
            if (looksLikeColor) {
                target._mode = 0;
                target.color = this._valueParser.asColor(rawValue);
                return;
            }
            ...
            if (record.mode !== undefined) {
                target._mode = this._valueParser.asNumber(record.mode, `${apiName}.mode`);
            } else if (typeof target._mode !== 'number') {
                target._mode = 0;
            }
            if (record.color !== undefined) {
                target.color = this._valueParser.asColor(record.color);
            }
            if (record.colorMin !== undefined) {
                target.colorMin = this._valueParser.asColor(record.colorMin);
            }
            if (record.colorMax !== undefined) {
                target.colorMax = this._valueParser.asColor(record.colorMax);
            }
            return;
        }
    ...
    private _encodeGradient(
        ...
        for (const [index, item] of colorKeysRaw.entries()) {
            ...
            colorKeys.push({
                ...
                time,
                color: this._valueParser.asColor(record.color),
            });
        }
    ...
    private _applyEncoded(
        ...
        if ((apiName === 'width' || apiName === 'height') && spec.serializedName === '_contentSize') {
            const size = this._valueParser.asSize(existing[spec.serializedName]);
            if (apiName === 'width') {
                size.width = this._valueParser.asNumber(rawValue, apiName);
            } else {
                size.height = this._valueParser.asNumber(rawValue, apiName);
            }
            encoded[spec.serializedName] = size;
            return;
        }
        if ((apiName === 'anchorX' || apiName === 'anchorY') && spec.serializedName === '_anchorPoint') {
            const point = this._valueParser.asVec2(existing[spec.serializedName]);
            if (apiName === 'anchorX') {
                point.x = this._valueParser.asNumber(rawValue, apiName);
            } else {
                point.y = this._valueParser.asNumber(rawValue, apiName);
            }
    ...
    private _encodeValue(
        ...
        switch (kind) {
            ...
            case 'number':
            case 'enum':
                return this._valueParser.asNumber(value, apiName);
            case 'boolean':
            ...
            case 'vec2':
                return this._valueParser.asVec2(value);
            case 'vec3':
                return this._valueParser.asVec3(value);
            case 'rect':
                return this._valueParser.asRect(value);
            case 'color':
                return this._valueParser.asColor(value);
            case 'uuid':
                return this._valueParser.asUuidRef(value, apiName);
            case 'uuidList':
                return this._valueParser.asUuidList(value, apiName);
            case 'vec2List':
                return this._valueParser.asVec2List(value, apiName);
            case 'vec3List':
                return this._valueParser.asVec3List(value, apiName);
            case 'stringList':
                return this._valueParser.asStringList(value, apiName);
            case 'numberList':
                return this._valueParser.asNumberList(value, apiName);
            case 'nodeRef':
    ...
    private _decodeValue(
        ...
        switch (spec.kind) {
            ...
            case 'curveRange': {
                const target = this._referenceDecoder.resolveEmbeddedRaw(raw, resolveEntry);
                if (target == null) {
                    return null;
                }
                const keys = this._valueParser.decodeRealCurveKeys(target.spline);
                const keysMin = this._valueParser.decodeRealCurveKeys(target.splineMin);
                const keysMax = this._valueParser.decodeRealCurveKeys(target.splineMax);
                return {
                ...
                const mode =
                    typeof target._mode === 'number'
                        ...
                        : typeof target.mode === 'number'
                          ? target.mode
                          : 0;
                const colorKeys = this._valueParser.decodeGradientColorKeys(target.gradient);
                const colorKeysMin = this._valueParser.decodeGradientColorKeys(target.gradientMin);
                const colorKeysMax = this._valueParser.decodeGradientColorKeys(target.gradientMax);
                return {
                    mode,
            ...
            case 'objectList': {
                ...
                return raw.map((item) => {
                    const target = this._referenceDecoder.resolveEmbeddedRaw(item, resolveEntry);
                    if (target == null) {
                        return null;
                    }
                    const nested: Record<string, unknown> = {};
            ...
            case 'eventHandlerList': {
                ...
                return raw.map((item) => {
                    ...
                    return {
                        target: this._referenceDecoder.decodeIdRef(target.target, pathForNodeIndex, null),
            ...
            case 'nodeRef':
                return this._referenceDecoder.decodeIdRef(raw, pathForNodeIndex, null);
            case 'componentRef':
                return this._referenceDecoder.decodeIdRef(raw, pathForNodeIndex, spec.refComponentType ?? null);
            case 'nodeRefList':
                if (!Array.isArray(raw)) {
                    return [];
                }
                return raw.map((item) => this._referenceDecoder.decodeIdRef(item, pathForNodeIndex, null));
            case 'componentRefList':
                if (!Array.isArray(raw)) {
                    return [];
                }
